...
impl CommandInjectionScanner {
    pub fn new() -> Self {
        Self {
            payloads: PayloadSet::for_command_injection(),
        }
    }
    ...
    async fn test_parameter(
        ...
        for payload in &self.payloads.payloads {
            ...
            debug!("Testing Command Injection with payload: {} on {}", payload.description, test_url);

            match client.get(&test_url).await {
                Ok(response) => {
                    // Check for command execution indicators
                    if self.is_vulnerable(&response.body, &payload.detection_patterns)
                        || self.has_timing_anomaly(&response) {
                        vulnerabilities.push(Vulnerability {
                            ...
                            description: format!(
                                "Command Injection detected using {}. The application may be executing system commands with user input.",
                                payload.description
                            ),
                            remediation: "Never pass user input directly to system commands. Use allowlists for valid inputs. Avoid shell execution entirely if possible. Use language-specific APIs instead.".to_string(),
    ...
    fn has_timing_anomaly(&self, response: &crate::client::HttpResponse) -> bool {
        // If response took longer than 5 seconds, might be sleep-based injection
        response.response_time.as_secs() >= 5
    }
...
impl VulnerabilityScanner for CommandInjectionScanner {
    ...
    async fn scan(&self, client: &HttpClient, url: &str) -> Result<Vec<Vulnerability>> {
        ...
        if params.is_empty() {
            let test_params = vec!["cmd", "exec", "command", "ping", "host"];
