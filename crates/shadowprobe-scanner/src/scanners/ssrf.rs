...
impl SsrfScanner {
    ...
    async fn test_parameter(
        ...
        for payload in &self.payloads.payloads {
            ...
            debug!("Testing SSRF with payload: {} on {}", payload.description, test_url);
                ...
                Ok(response) => {
                    if self.is_vulnerable(&response.body, &payload.detection_patterns) {
                        vulnerabilities.push(Vulnerability {
                            ...
                            description: format!(
                                "Server-Side Request Forgery (SSRF) detected using {}. The application may allow attackers to make requests to internal resources.",
                                payload.description
                            ),
                            remediation: "Use a whitelist of allowed protocols and domains. Validate and sanitize all URLs. Never trust user-provided URLs. Implement network segmentation.".to_string(),
...
impl VulnerabilityScanner for SsrfScanner {
    ...
    async fn scan(&self, client: &HttpClient, url: &str) -> Result<Vec<Vulnerability>> {
        ...
        if params.is_empty() {
            let test_params = vec!["url", "uri", "redirect", "link", "target"];
